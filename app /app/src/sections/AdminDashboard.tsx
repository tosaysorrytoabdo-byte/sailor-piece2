...
function useStoreContact() {
  const [contact, setContact] = useState(() => {
    try {
      const saved = localStorage.getItem('slr-contact');
      return saved ? JSON.parse(saved) : { tiktok: defaultConfig.tiktok, whatsapp: defaultConfig.whatsapp };
    } catch { return { tiktok: defaultConfig.tiktok, whatsapp: defaultConfig.whatsapp }; }
  });
  const save = (data: typeof contact) => {
    setContact(data);
    localStorage.setItem('slr-contact', JSON.stringify(data));
  };
  return { contact, save };
}

export { useStoreContact };

export default function AdminDashboard({ onExit }: { onExit?: () => void }) {
  const { orders, updateOrderStatus, deleteOrder, getPendingOrders, getCompletedOrders } = useOrders();
  const { products, addProduct, updateProduct, deleteProduct } = useProducts();
  const { contact, save: saveContact } = useStoreContact();
